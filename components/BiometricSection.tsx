...
import { Scan, Lock, Fingerprint } from 'lucide-react';

const BiometricSection: React.FC = () => {
  const [activePhase, setActivePhase] = useState(0);
  const phases = [
    { label: 'Add Reference', desc: 'Guided visual capture' },
    { label: 'Build Anchor', desc: 'Creating creative references' },
    { label: 'Guide Output', desc: 'Ready for generation' },
  ];
        ...
        <div className="flex flex-col md:flex-row gap-16 items-center">
          <div className="flex-1 space-y-8">
            ...
            <h3 className="text-4xl font-bold font-sans">
              Build Better <br />
              <span className="text-gray-500">Visual Foundations.</span>
            </h3>
            <p className="text-gray-400 leading-relaxed text-lg">
              Reference-guided capture helps creators build stronger visual
              anchors for themselves, clients, spokespeople, characters, and
              brand personalities. It is designed for creative direction, not
            ...
            <div className="space-y-6">
              <div className="flex gap-4 items-start p-4 rounded-lg bg-white/5 border border-white/5 hover:border-nano-yellow/30 transition-colors">
                ...
                <div>
                  <h4 className="text-white font-bold mb-1">Stronger Visual Anchors</h4>
                  <p className="text-sm text-gray-400">Use source references to communicate visual intent with less prompt repetition.</p>
                </div>
              </div>
              <div className="flex gap-4 items-start p-4 rounded-lg bg-white/5 border border-white/5 hover:border-nano-yellow/30 transition-colors">
                <div className="bg-nano-yellow/10 p-3 rounded text-nano-yellow">
                ...
                <div>
                  <h4 className="text-white font-bold mb-1">Style Exploration</h4>
                  <p className="text-sm text-gray-400">Explore visual directions from photorealism to anime, illustration, and stylized looks.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 w-full relative">
            <div className="w-full aspect-[4/5] bg-black border border-nano-border rounded-lg relative overflow-hidden flex flex-col">
              <div className="h-12 border-b border-nano-border flex items-center px-4 justify-between bg-nano-dark">
                ...
                <span className="font-mono text-xs text-gray-500">SESSION: REF-GEN-09</span>
              </div>
              ...
              <div className="flex-1 relative bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-gray-800 to-black">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <motion.div className="relative w-48 h-64">
                    <motion.div
                    ...
                    {[0, 1, 2, 3].map((i) => (
                      <motion.div
                        ...
                        style={{ rotate: i * 45 }}
                      >
                        <div className="absolute top-0 left-1/2 w-3 h-3 bg-white/20 rounded-full backdrop-blur-sm transform -translate-x-1/2 -translate-y-1/2" />
                      </motion.div>
                    ))}
                  </motion.div>
                </div>
                ...
                <div className="absolute bottom-8 left-8 right-8">
                  <div className="flex justify-between mb-2">
                    <span className="font-mono text-nano-yellow text-xs">BUILDING REFERENCE...</span>
                    <span className="font-mono text-white text-xs">{activePhase === 0 ? 'INPUT' : activePhase === 1 ? 'PROCESSING' : 'READY'}</span>
                  </div>
                  ...
                  <div className="mt-4 flex justify-between gap-2">
                    {phases.map((phase, idx) => (
                      <div key={phase.label} className={`text-[10px] font-mono uppercase transition-colors ${idx === activePhase ? 'text-white font-bold' : 'text-gray-600'}`}>
                        {phase.label}
                      </div>
                    ))}
                  </div>
